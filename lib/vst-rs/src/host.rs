...
impl Plugin for PluginInstance {
    ...
    fn get_editor(&mut self) -> Option<Box<dyn Editor>> {
        let mut rect: MaybeUninit<super::editor::Rect> = MaybeUninit::uninit();
        let ptr = rect.as_mut_ptr() as *mut c_void;
        self.dispatch(plugin::OpCode::EditorGetRect, 0, 0, ptr, 0.0);
        let rect = unsafe { rect.assume_init() };
        Some(Box::new(HostEditor::new(rect)))
    }

    fn open_editor(&mut self, handle: *mut c_void) {
        self.dispatch(plugin::OpCode::EditorOpen, 0, 0, handle, 0.0);
    }
}
...
impl Editor for HostEditor {
    ...
    fn open(&mut self, handle: *mut c_void) -> bool {
        true
    }
